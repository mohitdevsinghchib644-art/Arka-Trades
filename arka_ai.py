...
# ══════════════════════════════════════════════════════════
# 2. EMBEDDING — uses REST API directly (most reliable)
# ══════════════════════════════════════════════════════════

def get_embedding(text: str) -> list:
    """
    Get text embedding via Google REST API directly.
    More reliable than SDK wrappers across all versions.
    """
    ...
    try:
        ...
        if resp.status_code == 200:
            ...
            st.error(f"Embedding API error {resp.status_code}: {resp.text[:200]}")
            return None
    except Exception as e:
...
def generate_chart_notes(img: "Image.Image", user_note: str = "") -> str:
    """
    Ask Gemini to describe what it sees in the chart.
    Returns AI-generated observations as a string.
    Called BEFORE saving to memory so notes are enriched.
    """
...
def verdict_badge(verdict: str, score: int) -> str:
    ...
    return f"""
<div style="background:{DARK2};border:1px solid {c};border-radius:14px;
    ...
    with col_panel:
        ...
        if st.button("ANALYZE CHART", type="primary",
            ...
            if result.get("draw_boxes") or result.get("draw_arrows"):
                ...
                with col_chart:
                    st.image(annotated, caption="AI Annotated Chart",
...
def render_mode2():
    ...
    with tab_manual:
        ...
        rule_name = st.text_input(
            ...
            key="m2_manual_rule_name"
        )
        rule_text = st.text_area(
            "Exact Conditions",
            placeholder="e.g. Price must close above PDH. Volume must be 1.5x average. RSI above 55.",
            height=130,
            key="m2_manual_rule_text"
        )
        if st.button("SAVE TO MEMORY", use_container_width=True,
            ...
            if rule_name.strip() and rule_text.strip():
                with st.spinner("Saving to Pinecone..."):
                    ok = save_rule_to_memory("Manual", rule_name.strip(),
                                             rule_text.strip(), [])
                if ok:
    ...
    with tab_chart:
        st.markdown(
            ...
            f"Upload a setup chart. AI auto-reads the image AND your notes. "
            f"Click on key zones to add coordinate context.</div>",
            unsafe_allow_html=True
        )
        ...
        if train_img_file:
            ...
            if HAS_IMG_COORDS:
                st.markdown(
                    ...
                    f"Click on key candle or zone to lock coordinates</div>",
                    unsafe_allow_html=True
                )
                train_coords = streamlit_image_coordinates(
                    raw_img, key="train_click", use_column_width=True
                )
        ...
        setup_name = st.text_input(
            "Setup Name",
            placeholder="e.g. Low Volume Consolidation Handle",
            key="m2_setup_name_input"
        )
        setup_rules = st.text_area(
            "Your observations (optional — AI will also read the chart itself)",
            placeholder="e.g. Volume drops 40% during consolidation. Entry on breakout above left cup rim.",
            height=100,
            key="m2_setup_rules_input"
        )
            ...
            if setup_name.strip():
                if not train_img_file:
                    ...
                    with st.spinner("AI reading chart image and saving to memory..."):
                        ...
                        # ── STEP 1: AI auto-generates notes from the image
                        raw_img_for_notes = Image.open(train_img_file).convert("RGB")
                        ai_notes = generate_chart_notes(
                            raw_img_for_notes,
                            user_note=setup_rules.strip()
                        )

                        # ── STEP 2: Combine AI notes + user notes
                        combined = ""
                        ...
                        # ── STEP 3: Add coordinate context if clicked
                        if tx:
                            combined += (
                                f"\n\nCHART COORDINATE: ({tx},{ty}) = "
                                f"{round(tx/orig_w*100,1)}% X, "
                                f"{round(ty/orig_h*100,1)}% Y"
                            )
    ...
    with tab_memory:
        ...
        query = st.text_input(
            "Search memory",
            placeholder="e.g. volume breakout consolidation",
            key="mem_search"
        )
        ...
        if not results:
            ...
            st.markdown(
                f"<div style='font-size:12px;color:{T2};margin-bottom:12px;'>"
                f"Showing {len(results)} rules:</div>",
                unsafe_allow_html=True
            )
            for r in results:
