...
data={"chat_id","text","parse_mode":"HTML"}, timeout=5)
except: pass
...
f'style="vertical-align;">{_ICON_PATHS.get(name,"")}</svg>')
...
for k, v in {"logged_in","disclaimer_done","show_login","page":"home",
"profile":{"name":"Trader","email":"","phone":""},"profile_photo","watchlist":[],
"admin_watchlist":[],"alerts":{},"alert_fired"(),"db_loaded","is_admin",
"active_news_source":"admin","show_news_rail","active_security":""}.items():
...
if k.startswith(("results", "failed_", "alert_open_", "open_")):
st.session_state.pop(k, None)
...
def render_compact_module_header(title, code=""):
c1, c2, c3 = st.columns([2.4, 5.5, 1.5])
with c1:
if st.button("▲  ARKA TRADES", key=f"brand_home{code}{title}", use_container_width=True):
go_home()
with c2:
st.markdown(
...
with c3:
if st.button("← BACK TO TERMINAL", key=f"back_terminal{code}{title}", use_container_width=True):
_go_home()
...
def render_terminal_header(show_indices=True):
...
global = [("S&P 500", "^GSPC", None), ("NASDAQ", "^IXIC", None), ("DOW", "^DJI", None), ("DAX", "^GDAXI", None), ("FTSE", "^FTSE", None), ("NIKKEI", "^N225", None)]
st.markdown('<div class="terminal-shell">', unsafe_allow_html=True)
render_terminal_brandbar()
if show_indices:
for title, items in (("INDIA", india), ("GLOBAL", global)):
...
def render_module_dock(active=None):
...
if st.button("OPEN", key=f"module_v5{target}", use_container_width=True):
...
if pg == "home":
render_dashboard()
render_module_directory()
elif pg == "security":
active = st.session_state.get("active_security", "")
if not active:
render_dashboard()
else:
render_security_workspace(active)
elif pg == "scanner":
...
filt=sc1.selectbox("Show",["All","Above PDH","Below PDL","In Range"],key=f"filt_v4{key_prefix}")
refresh_after=sc2.checkbox("Refresh after scan",key=f"refresh_after{key_prefix}")
sc3.caption("Cached")
scanbtn=sc4.button("Run Scan",use_container_width=True,type="primary",key=f"scan_v4{key_prefix}")
if scanbtn:
st.session_state["active_news_source"]=key_prefix
results,failed=[],[]
bar=st.progress(0,text="Scanning...")
for i,sym in enumerate(syms):
st=get_static(sym); lv=get_price(sym)
if st_ and lv:
cur=lv["price"]; chg=lv["chg"]
cls="g" if cur>st_["pdh"] else "r" if cur<st_["pdl"] else "n"
results.append({"sym","cur","chg","pdh":st_["pdh"],"pdl":st_["pdl"],"rsi":st_["rsi"],"cls","spark":st_.get("spark",[])})
...
with cols7[i%4]:
if st.button("OPEN",key=f"open_scan_{key_prefix}{s['sym']}",use_container_width=True): open_security(s["sym"])
...
active_alerts={s for s,a in st.session_state.alerts.items() if a.get("active")}
...
if c4.button("OFF" if active else "SET",key=f"alertact{suffix}{sym}"):
if active:
del st.session_state.alerts[sym]; db_delete_alert(sym); st.rerun()
else:
st.session_state[f"alert_open_{suffix}{sym}"]=True; st.rerun()
if st.session_state.get(f"alert_open{suffix}{sym}"):
typ=st.radio("Condition",["PDH","PDL","Custom"],horizontal=True,key=f"atype{suffix}{sym}")
cp=st.number_input("Trigger price",min_value=0.0,step=0.5,key=f"acp{suffix}{sym}") if typ=="Custom" else 0.0
x,y=st.columns(2)
if x.button("Cancel",key=f"acan{suffix}{sym}"): st.session_state[f"alert_open{suffix}{sym}"]=False; st.rerun()
if y.button("Confirm",key=f"acon{suffix}{sym}",type="primary"):
...
if price:
at=typ.lower(); st.session_state.alerts[sym]={"type","price","active"}; db_save_alert(sym,at,price); st.session_state[f"alert_open{suffix}{sym}"]=False; st.rerun()
...
render_research_page(TERM_TOKENS,news_fetch_fn=fetch_news_for_stock)
elif pg == "analysis":
...
with p2:
with st.form("pf_v4"):
a,b=st.columns(2); nn=a.text_input("Full Name",value=st.session_state.profile["name"]); np=b.text_input("Contact Number",value=st.session_state.profile["phone"]); ne=st.text_input("Email Address",value=st.session_state.profile["email"]); ph=st.file_uploader("Upload Profile Photo",type=["jpg","jpeg","png"],key="profile_photo_v4")
if st.form_submit_button("Save Profile",use_container_width=True,type="primary"):
st.session_state.profile.update({"name","phone":np,"email"});
