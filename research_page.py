...
def render_research_page(T=None, news_fetch_fn=None):
    ...
    # Main security chart remains above the deep research tabs.
    _chart(data["symbol"],T,"6mo")
    st.markdown(f'<div style="border-bottom:1px solid {T["border"]};margin:12px 0"></div>',unsafe_allow_html=True)
    tabs=["Overview","Financials","Earnings","Valuation","Ownership","Flows","Segments","Peers","Supply Chain","Management","Events","News","Risk","Factors","Price + Fundamentals","What Changed?","Thesis","Research Graph"]
    if option_menu:
        selected=option_menu(None,tabs,icons=["grid","bar-chart","graph-up","cash","people","activity","diagram-3","columns","truck","mic","calendar","newspaper","shield","bullseye","graph-up-arrow","lightning","journal-text","diagram-2"],default_index=0,orientation="horizontal",styles={"container":{"padding":"0!important","background-color":"transparent"},"icon":{"color":T["amber"],"font-size":"12px"},"nav-link":{"font-size":"9px","text-align":"center","margin":"0px","padding":"7px 4px","font-family":T["mono"]},"nav-link-selected":{"background-color":"transparent","color":T["amber"],"border-bottom":f"2px solid {T["amber"]}"}})
    else:
        selected=st.selectbox("Research function",tabs,key="research_function")
    st.markdown('<div style="height:5px"></div>',unsafe_allow_html=True)
    dispatch={
        "Overview":lambda:_overview(data,T),"Financials":lambda:_financials(data,T),"Earnings":lambda:_earnings(data,T),"Valuation":lambda:_valuation(data,T),
        "Ownership":lambda:_ownership(data,T),"Flows":lambda:_flows(data,T),"Segments":lambda:_segments(data,T),"Peers":lambda:_peers(data,T),
        "Supply Chain":lambda:_supply_chain(data,T),"Management":lambda:_management(data,T),"Events":lambda:_events(data,T),"News":lambda:_news(data,T,news_fetch_fn),
        "Risk":lambda:_risk(data,T),"Factors":lambda:_factors(data,T),"Price + Fundamentals":lambda:_price_fundamentals(data,T),"What Changed?":lambda:_what_changed(data,T),
        "Thesis":lambda:_thesis(data,T),"Research Graph":lambda:_graph(data,T),
    }
    dispatch[selected]()
